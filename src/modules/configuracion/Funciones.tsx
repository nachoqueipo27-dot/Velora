...
import { FUNCIONES_DISPONIBLES, TODOS_LOS_IDS, type FuncionDisponible } from '../onboarding/types'

const CheckBox = ({ state, onClick }: { state: 'on' | 'off' | 'partial'; onClick: () => void }) => (
  <button type="button" onClick={onClick}
    className={cn('w-4 h-4 rounded-[4px] border flex items-center justify-center shrink-0 transition-all duration-120',
      state !== 'off'
...
export const Funciones = () => {
  ...
  const toggleModule = (f: FuncionDisponible) => {
    ...
    setEnabled(turningOn ? Array.from(new Set([...enabled, ...ids])) : enabled.filter(id => !ids.includes(id)))
  }
  const toggleSub = (f: FuncionDisponible, subId: string) =>
    setEnabled(enabled.includes(subId) ? enabled.filter(id => id !== subId) : Array.from(new Set([...enabled, subId, f.id])))
  const toggleExpand = (id: string) => setExpanded(p => p.includes(id) ? p.filter(x => x !== id) : [...p, id])

  return (
    <div className="max-w-2xl flex flex-col gap-4">
      <header>
        <h2 className="text-lg font-semibold text-white light:text-black">Funciones</h2>
        <p className="text-[12px] text-[#606060]">Los módulos deshabilitados desaparecen de la navegación. Dashboard, Configuración y Uso siempre quedan visibles.</p>
      </header>

      <div className="flex justify-end">
        <Button size="sm" variant="secondary" onClick={() => setEnabled([...TODOS_LOS_IDS])}>Activar todas</Button>
      </div>

      <div className="flex flex-col rounded-card border overflow-hidden border-[#2A2A2A] light:border-[#E4E4E4]">
        {FUNCIONES_DISPONIBLES.map((f, idx) => {
          const hasSubs = f.subfunciones.length > 0
          const isExpanded = expanded.includes(f.id)
          return (
            <div key={f.id} className={cn(idx !== 0 && 'border-t border-[#2A2A2A] light:border-[#E4E4E4]')}>
              <div className="flex items-center gap-3 px-4 py-2.5">
                <CheckBox state={moduleState(f)} onClick={() => toggleModule(f)} />
                <button type="button" onClick={() => hasSubs && toggleExpand(f.id)}
                  className={cn('flex-1 flex items-center justify-between text-left text-sm text-white light:text-black', hasSubs ? 'cursor-pointer' : 'cursor-default')}>
                  <span>{f.label}</span>
                  {hasSubs && <span className="text-[#606060]">{isExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}</span>}
                </button>
              </div>
              {hasSubs && isExpanded && (
                <div className="flex flex-col pb-2">
                  {f.subfunciones.map(s => (
                    <div key={s.id} className="flex items-center gap-3 pl-11 pr-4 py-1.5">
                      <CheckBox state={enabled.includes(s.id) ? 'on' : 'off'} onClick={() => toggleSub(f, s.id)} />
