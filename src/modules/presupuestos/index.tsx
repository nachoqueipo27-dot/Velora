import { useState } from 'react'
import { cn } from '../../lib/utils'
import { usePresupuestosStore } from '../../store/presupuestosStore'
import type { Presupuesto } from '../../types/presupuestos'
import { ListadoPresupuestos } from './ListadoPresupuestos'
import { DetallePresupuesto } from './DetallePresupuesto'
import { ModalPresupuesto } from './ModalPresupuesto'
import { FileText, FilePlus, History } from 'lucide-react'

type Seccion = 'listado' | 'historial'

const SUBMENU: { id: Seccion | 'crear'; label: string; icon: React.ReactNode }[] = [
  { id: 'listado',   label: 'Listado',            icon: <FileText size={15} /> },
  { id: 'crear',     label: 'Crear presupuesto',  icon: <FilePlus size={15} /> },
  { id: 'historial', label: 'Historial',          icon: <History size={15} /> },
]

const Presupuestos = () => {
  const { presupuestoActivo, seleccionar, cargarItems } = usePresupuestosStore()
  const [seccion, setSeccion] = useState<Seccion>('listado')
  const [nuevo, setNuevo] = useState(false)
  const [editTarget, setEditTarget] = useState<Presupuesto | null>(null)

  const ver = (p: Presupuesto) => { seleccionar(p); cargarItems(p.id) }

  const irA = (id: Seccion | 'crear') => {
    seleccionar(null)
    if (id === 'crear') { setSeccion('listado'); setNuevo(true) }
    else setSeccion(id)
  }

  const renderContenido = () => {
    if (presupuestoActivo) {
      return <DetallePresupuesto onVolver={() => seleccionar(null)} onEditar={(p) => setEditTarget(p)} />
    }
    return (
      <ListadoPresupuestos
        onVer={ver}
        modo={seccion === 'historial' ? 'historial' : 'todos'}
        abrirNuevo={nuevo}
        onNuevoAbierto={() => setNuevo(false)}
      />
    )
  }

  return (
    <div className="flex h-full overflow-hidden">
      <aside className={cn('w-48 shrink-0 border-r flex flex-col py-4', 'border-[#2A2A2A] light:border-[#E4E4E4]')}>
        <span className="px-4 mb-3 text-[11px] font-semibold uppercase tracking-wider text-[#606060]">Presupuestos</span>
        <nav className="flex flex-col">
          {SUBMENU.map(item => {
            const activo = item.id === seccion || (item.id === 'listado' && seccion === 'listado')
            return (
              <button key={item.id} onClick={() => irA(item.id)}
                className={cn('flex items-center gap-2.5 px-4 py-2 text-[13px] text-left transition-all duration-150',
                  activo
                    ? 'border-l-2 border-white text-white pl-[14px] bg-white/[0.04] light:border-black light:text-black light:bg-black/[0.03]'
                    : 'border-l-2 border-transparent text-[#A0A0A0] hover:text-white hover:bg-white/[0.03] light:text-[#404040] light:hover:text-black light:hover:bg-black/[0.02]')}>
                {item.icon}<span>{item.label}</span>
              </button>
            )
          })}
        </nav>
      </aside>
      <section className="flex-1 overflow-hidden p-6">{renderContenido()}</section>
      <ModalPresupuesto open={editTarget !== null} presupuesto={editTarget} onClose={() => setEditTarget(null)} />
    </div>
  )
}

export default Presupuestos
